...
type TrustZoneClient interface {
	...
	ListTrustZones(ctx context.Context /*orgID string*/) ([]*trustzonepb.TrustZone, error)
...
func (c *trustZoneClient) CreateTrustZone(ctx context.Context, trustZone *trustzonepb.TrustZone) (*trustzonepb.TrustZone, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create trust zone %s: %w", trustZone.GetId(), err)
	}
...
func (c *trustZoneClient) GetTrustZone(ctx context.Context, trustZoneID string) (*trustzonepb.TrustZone, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get trust zone %s: %w", trustZoneID, err)
	}

	return resp.TrustZone, nil
}

func (c *trustZoneClient) ListTrustZones(ctx context.Context /*orgID string*/) ([]*trustzonepb.TrustZone, error) {
	resp, err := c.trustZoneClient.ListTrustZones(ctx, &trustzonesvcpb.ListTrustZonesRequest{
		//OrgId: orgID,
	})
	if err != nil {
		return nil, fmt.Errorf("failed to list trust zones: %w", err)
	}
...
func (c *trustZoneClient) RegisterAgent(ctx context.Context, agent *trustzonesvcpb.Agent, token string, bundle *types.Bundle) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to register agent in cluster %s in trust zone %s: %w", agent.ClusterId, agent.TrustZoneId, err)
	}
